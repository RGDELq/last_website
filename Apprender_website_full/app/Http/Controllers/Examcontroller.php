        ...
        if(count($qnaExam) > 0){
            if($qnaExam[0]['date'] == date('Y-m-d')){
              if(count($qnaExam[0]['getQnaExam']) > 0)
              {
                ...
                return view('student.exam-dashboard',['success'=>false,'msg'=>'This examis not availble ','exam'=>$qnaExam]);

              }
            }
            elseif
            ($qnaExam[0]['date'] > date('Y-m-d'))
            {
                return view('student.exam-dashboard',['success'=>false,'msg'=>'This exam will sart at '.$qnaExam[0]['date'],'exam'=>$qnaExam]);
            }
            else{
                return view('student.exam-dashboard',['success'=>false,'msg'=>'This exam expire '.$qnaExam[0]['date'],'exam'=>$qnaExam]);
