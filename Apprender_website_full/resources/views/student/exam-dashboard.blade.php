...
<div  style= 'background-color: snow'class='container'>

    <p style="color:rgb(97, 50, 112);">Welcome, {{ Auth::user()->name }}</p>
    ...
    <div>
         ...
         <input type="hidden" name="ans_{{$qcount-1}}" id="ans_{{$qcount-1}}">
              ...
              {{$acount++}} </b>{{ $answer['answer'] }}
              <p>  <input type="radio" name="radio_{{$qcount-1}}" data-id="{{$qcount-1}}" class="select_ans" value="{{ $answer['id'] }}">
    ...
    @else
    <h3 style="color:red;" class="text-center">question and answers are not availble</h3>
@endif
